...
//Função que popula um TClientDataSet a partir de um componente tipo Query(TZQuery,TFQuery,TQuery,etc...).
  case aTipoQuery of
    tqFiredac:
     begin
       (aQuery.Query as TFDQuery).First;
       aCds.Append;
       while not (aQuery.Query as TFDQuery).Eof do
       begin
         for field in (aQuery.Query as TFDQuery).Fields do
           aCds.Fields[field.Index].Value := field.Value;
         aCds.Post;
         (aQuery.Query as TFDQuery).Next;
       end;
     end;
     tqZeos:
     begin
       (aQuery.Query as TZQuery).First;
        aCds.Append;
        while not (aQuery.Query as TZQuery).Eof do
        begin
          for field in (aQuery.Query as TZQuery).Fields do
            aCds.Fields[field.Index].Value := field.Value;
          aCds.Post;
         (aQuery.Query as TZQuery).Next;
        end;
      end;
      tqRDW:
      begin
        (aQuery.Query as TRESTDWClientSQL).First;
        aCds.Append;
        while not (aQuery.Query as TRESTDWClientSQL).Eof do
        begin
          for field in (aQuery.Query as TRESTDWClientSQL).Fields do
            aCds.Fields[field.Index].Value := field.Value;
          aCds.Post;
         (aQuery.Query as TRESTDWClientSQL).Next;
        end;
      end;
      tqDbExpress:
       ...
       for prop in tipo.GetProperties do
         ...
         begin
           if atributo is TChavePrimaria then
             aCds.FieldDefs.Add(TChavePrimaria(atributo).Nome, ftInteger);
           if atributo is TCampoTexto then
             aCds.FieldDefs.Add(TCampoTexto(atributo).Nome, ftString, TCampoTexto(atributo).Tamanho);
           if atributo is TCampoInteiro then
             aCds.FieldDefs.Add(TCampoInteiro(atributo).Nome, ftInteger);
           if atributo is TCampoData then
             aCds.FieldDefs.Add(TCampoData(atributo).Nome, ftDate);
           if atributo is TCampoDataHora then
             aCds.FieldDefs.Add(TCampoDataHora(atributo).Nome, ftDate);
           if atributo is TCampoExtended then
             aCds.FieldDefs.Add(TCampoExtended(atributo).Nome, ftExtended);
           if atributo is TCampoMonetario then
             aCds.FieldDefs.Add(TCampoMonetario(atributo).Nome, ftCurrency);
           if atributo is TCampoBooleano then
              aCds.FieldDefs.Add(TCampoBooleano(atributo).Nome, ftBoolean);
           if atributo is TChaveEstrangeira then
             aCds.FieldDefs.Add(TChaveEstrangeira(atributo).Nome, ftInteger);
         end;
         aCds.CreateDataSet;
     finally
       contexto.Free;
...
{class function TLib<T>.ListaparaJsonArray(aLista: TObjectList<T>): TJSONArray;
...
begin
   ...
   try
     for i := 1 to aLista.Count do
     begin
       json := ;
       json_array.AddElement(json);
     end;
   finally
     json.Free;
     json_array.Free;
...
class function TLib<T>.StreamParaString(aStream: TStream): string;
...
class procedure TLib<T>.StringParaStream(const aValor: string;
  aStream: TStream);
var StringStream: TStringStream;
begin
   aStream.Size := 0;
   StringStream := TStringStream.Create(aValor, TEncoding.UTF8);
   try
