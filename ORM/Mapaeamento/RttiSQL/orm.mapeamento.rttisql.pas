...
function TRttiSQL<T>.LastId(var aSql: string): iRttiSQL<T>;
var aNomeClasse, aPK: string;
begin
   Result := Self;
   TRttiUtils<T>.New(FInstance).NomeTabela(aNomeClasse).ChavePrimaria(aPK);
   aSql := aSql + 'SELECT FIRST(1) ' + aPK;
   aSql := aSql + ' FROM ' + aNomeClasse;
   aSql := aSql + ' ORDER BY ' + aPK + ' DESC';
end;

function TRttiSQL<T>.LastRecord(var aSql: string): iRttiSQL<T>;
var aPK, aNomeClasse, aCampos: string;
begin
   Result := Self;
   TRttiUtils<T>.New(FInstance).NomeTabela(aNomeClasse).Fields(aCampos).ChavePrimaria(aPK);
   aSql := aSql + 'SELECT FIRST(1) ' + aCampos;
   aSql := aSql + ' FROM ' + aNomeClasse;
   aSql := aSql + ' ORDER BY ' + aPK + ' DESC';
end;
...
begin
   Result := Self;
   TRttiUtils<T>.New(FInstance).NomeTabela(aNomeTabela).Update(aCampos).Where(aWhere);

   aSql := aSql + 'UPDATE ' + aNomeTabela;
   aSql := aSql + ' SET ' + aCampos;
   aSql := aSql + ' WHERE ' + aWhere;
end;
